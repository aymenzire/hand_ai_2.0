...
# TODO: calibrer ces seuils selon ta caméra + ta main
THRESHOLDS = {
    "index":  [(145, 180), (110, 160), (0, 105)],
    "middle": [(150, 180), (115, 170), (0, 105)],
    "ring":   [(155, 180), (115, 145), (0, 105)],
    "pinky":  [(155, 180), (115, 145), (0, 105)],
    "thumb":  [(165, 180), (105, 160), (0, 100)],
}
...
def overlay_debug_info(frame, finger_angles, stable_count):
    """
    Affiche un overlay debug sur l'image:
    - angle de chaque doigt
    - compteur de stabilité (anti-bruit)
    """
    y = 25
    cv2.putText(
        frame,
        "DEBUG: Finger angles & stability",
        (10, y),
    ...
    for finger_name, angle_deg in finger_angles.items():
        max_stable = max(
            stable_count[(finger_name, 0)],
            stable_count[(finger_name, 1)],
            stable_count[(finger_name, 2)],
        )

        text = f"{finger_name}: {angle_deg:.1f} deg | stable={max_stable}"
        cv2.putText(
            frame,
            text,
            (10, y),
    ...
    # TODO: ajouter un code couleur selon l'etat (open / mid / close)
